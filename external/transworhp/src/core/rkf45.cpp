...
//
{
  ...
  f ( t + ch, f5, f1,  t0,  tend, u1, u2, param, n_ctrl);
  ...
  f ( t + 3.0 * h / 8.0, f5, f2 ,  t0,  tend, u1, u2, param, n_ctrl);
  ...
  f ( t + 12.0 * h / 13.0, f5, f3,  t0,  tend, u1, u2, param, n_ctrl );
  ...
  f ( t + h, f5, f4,  t0,  tend, u1, u2, param, n_ctrl );
  ...
  f ( t + h / 2.0, f1, f5,  t0,  tend, u1, u2, param, n_ctrl );
//
...
int r8_rkf45 ( void f ( double t, double y[], double yp[], double t0, double tend, double *u1, double *u2, double *param, int n_ctrl ), 
			  int neqn, 
  double y[], double yp[], double *t, double t0, double tout, double *u1, double *u2, double *param, double *relerr, 
  double abserr, int flag, int n_ctrl, int &stepcount )
...
//
  ...
  if ( mflag == 1 )
  {
    init = 0;
    kop = 0;
    f ( *t, y, yp, t0, tout, u1, u2, param, n_ctrl);
    nfe = 1;
...
//
      r8_fehl ( f, neqn, y, *t, h, yp, f1, f2, f3, f4, f5, f1,  t0, tout, u1, u2, param, n_ctrl );
      nfe = nfe + 5;
//
