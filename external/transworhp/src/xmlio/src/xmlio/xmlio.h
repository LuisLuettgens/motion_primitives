...
public:
	...
	/** Copy all attributes of other node. */
	void CopyAttributes(XMLNode *other) {
	...
	/** Add child node after par */
	XMLNode *AddChildAfter(XMLNode * n, XMLNode * par);
	...
	/** Insert node as child in current. */
	void Insert(XMLNode *n) {
		child.push_back(n);
	}

	/// Insert node as child in current at local tmp iterator
	void InsertHere(XMLNode *n);
	...
	/** Range Iterators
	*/
	std::vector<XMLNode*>::const_iterator begin();
	std::vector<XMLNode*>::const_iterator operator++();
	std::vector<XMLNode*>::const_iterator end();
