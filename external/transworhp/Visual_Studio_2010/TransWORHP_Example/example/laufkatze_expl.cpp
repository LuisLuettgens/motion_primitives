...
public:
	...
	XMLNode* scenenode;
#ifdef RB
	Laufkatze_ExplPhase(XMLNode *xmlmain, int dis, vector<int> &multinode) : ExplTransWorhp("Laufkatze (expl)", dis, multinode, 9,2,1,9,0) {
#else
	Laufkatze_ExplPhase(XMLNode *xmlmain, int dis, vector<int> &multinode) : ExplTransWorhp("Laufkatze (expl)", dis, multinode, 9,2,1,0,0,9) {

		// const char *s, int dis, vector<int> &multinode_, int ode, int ctrl, int param, int rand, int neben, int BOXneben, int integral, int zen)
#endif
		freetime = 1;
		grav = 9.81;
...
//	multinode.push_back(twparameter.NDIS-1);
	...
	Laufkatze_ExplPhase ph(xml_laufkatze, twparameter.NDIS, multinode);
...
os << setw(10) << B;
	stringstream a;
	for (vector<int>::iterator it = multinode.begin();it<multinode.end();it++)
		a << *it << "_";
