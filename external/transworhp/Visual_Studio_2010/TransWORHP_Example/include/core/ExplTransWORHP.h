...
public:
	...
	*/
	ExplTransWorhp(const char *s, int dis, std::vector<int> &multinode, int ode, int ctrl, int param, int rand, int neben, int BOXneben=0, int integral=0, int zen=0);
	...
	* @param zen Number of non-trivial zen parameters
	*/
	ExplTransWorhp(const char *s, int dis, int multi, int ode, int ctrl, int param, int rand=0, int neben=0, int BOXneben=0, int integral=0, int zen=0);
	...
	*/
	...
	int u_index__(int dis, int ode) const;
...
//#endif
	...
	void GetControl(double *uu, double t);

	void GetBoundaryIndices(std::vector<int> &indices, int d);
protected:
	virtual double &setx(int dis, int ode);

public:
	/** Speicher und Struktur */
	void GetSpace(int delta1, int delta2);
	/** Verbindung zu WORHP */
	void Connect(const OptVar &o, const Params &p);
	/** Beschraenkungsschranken */
	void Boundary();
	/** Beschraenkungen */
	void Constraints();
	/** Beschraenkungen mit anderem G Vektor */
	void Constraints2(double *GG, int DGflag=0);
...
protected:
	...
	void DG_diff_rand(int k);

	void DG_diff_neben(double t, int dis);
	void DG_diff_neben_p(double t, int l, int dis);

public:
	double Objective(double ScaleObj);

	int DF_structure(WorhpMatrix *DF=0, int offset=0);
	void DF_calculate(WorhpMatrix &DF, double ScaleObj);

	int DG_structure(const TWfolder *f, WorhpMatrix *DG=0, int offset=0);
	void DG_calculate(TWfolder *f, WorhpMatrix &DG);

	int HM_structure(int, WorhpMatrix &DF, WorhpMatrix &DG, WorhpMatrix *HM=0, int offset=0);
	int HM_structure_ohne_Diag(int, WorhpMatrix &DF, WorhpMatrix &DG, WorhpMatrix *HM=0, int offset=0);
