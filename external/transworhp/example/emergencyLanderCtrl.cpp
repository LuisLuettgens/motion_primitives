...
class EmergencyLanding : public tw::TransWorhpProblem {
public:

	EmergencyLanding(int dis) : TransWorhpProblem(tw::TWdimension("Emergency landing - min. control", dis, 8, 2, 1, 0, 0)) {
		freetime = true;
	...
	std::string GetXTitle(int d) override {
		...
		if (d==6) return "ctrl 1 - aux";
		if (d==7) return "ctrl 2 - aux";
	}
	...
	double obj() override {
		// Objective function
		return x(n_dis-1, 6) + x(n_dis-1, 7);
	}
	...
	void ode(double *dx, double t, const double *x, const double *u, const double *p) override {
		...
		dx[6] = u[0]*u[0] * p[0]; //control 1 energy
		dx[7] = u[1]*u[1] * p[0]; //control 2 energy
...
/*		double rho; // x[3]
	...
	bool ode_diff_p(tw::DiffStructure& s, double t, const double *x, const double *u, const double *p, int index) override {
		...
		s(6, p_indexode(0) ) = u[0]*u[0];
		s(7, p_indexode(0) ) = u[1]*u[1];
	...
	void p_boundary(double *p_low, double *p_upp) override {
		p_low[0] = 100; // Useful bounds to time
		p_upp[0] = 300;
	}
