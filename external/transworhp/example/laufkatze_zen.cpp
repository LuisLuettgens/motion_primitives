...
class LaufkatzeZen : public TransWorhp {
public:
	double grav;
	XMLNode* scenenode;

	
	LaufkatzeZen(XMLNode *xmlmain, int dis) : TransWorhp("Laufkatze",
			dis,9,2,1,0,0,0,1) {
		freetime = 1;
		grav = 9.81;
	...
	void zen_init(double *zen) override {
	...
	void ode(double *dx, double t, const double *x, const double *u, const double *p) override {
		...
		dx[1] = x[4] * p[0];
		dx[2] = x[3] * p[0];
		dx[3] = (x[4] - (grav-x[7])*x[2]/(/*ZEN[0]+*/x[5]))  * p[0];
		dx[4] = u[0] * p[0];
		dx[5] = x[6] * p[0];
		dx[6] = x[7] * p[0];
		dx[7] = u[1] * p[0];
...
int main(int argv, char* argc[]) {
	...
	LaufkatzeZen ph(xml_laufkatze, twparameter.NDIS);
	folder.Add(&ph);
