/*----------------------------------------------------------------
 *
 *  Example: Splineproblem mit Gitteranpassung
 *
 *----------------------------------------------------------------*/
...
public:
	...
	SplinePhase(const tw::TWdimension &TWdim, double alpha) : TransWorhpProblem(TWdim), alpha(alpha) {}
...
int main(int argv, char* argc[]) {
	...
	const double alpha = (args["alpha"] != "") ? std::stod(args["alpha"]) : 1/6.;

	tw::TWdimension TWdim;
	TWdim.ID = "Spline mesh-refinement";
	TWdim.n_dis = twparameter.NDIS;
	TWdim.n_ode = 3;
	TWdim.n_ctrl = 1;

	SplinePhase ph(TWdim, alpha);
