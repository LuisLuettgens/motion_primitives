...
void ausweichen2d(tw::glObject *obj, double *x, double t);
double phasenPlot (int &len, int &ndgl, int &nsteuer, double *t,double *x, double *u, int &i, int &index);

class AusweichenPhase : public tw::TransWorhpProblem {
public:
	...
	AusweichenPhase(XMLNode *xmlmain, const tw::TWdimension &TWdata) : TransWorhpProblem(TWdata) {
	...
	void selectWindows(tw::Viewer *viewer) override {
		...
		viewer->AddStateView(0,"x_E");
		viewer->AddStateView(1,"y_E");
		viewer->AddStateView(2,"v");
		viewer->AddStateView(3,"phi");
		viewer->AddStateView(4,"omega_phi");
		viewer->AddStateView(5,"beta");
		viewer->AddStateView(6,"delta");
		viewer->AddStateView(7,"hilfe");

		viewer->AddControlView(0,"omega_delta");
		viewer->AddControlView(1,"F_B");
	}
...
int main(int argv, char* argc[]) {
	...
	AusweichenPhase ph(xml_node.get(), TWdim);
	...
	ph.solver->X[ph.x_index(0,2)] = 70;
	for (int i=0;i<twparameter.NDIS;i++) {
		double z = i/(twparameter.NDIS-1.);
		ph.solver->X[ph.u_index(i,0)] = (i>twparameter.NDIS/4 && i<twparameter.NDIS*0.75/4) ? -0.7 : 0.7 ;//6*(z-.5)*(z-.5) - .5;
		ph.solver->X[ph.u_index(i,1)] = 0;//-.6*(z-.5)*(z-.5) + .05;
	}
	ph.solver->X[ph.p_index(0)] = 1;
