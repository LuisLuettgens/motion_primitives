...
double roboterplot (int &len, int &ndgl, int &nsteuer, double *t,double *x, double *u, int &i, int &index) {
 ...
 if (index==0) return x[i*ndgl + 4]-x[i*ndgl + 5];
	...
	if (index==1) {
		int g_index = (tw_ph->n_dis-1) * tw_ph->n_ode + tw_ph->n_rand + i*tw_ph->n_neben;
		return tw_ph->G[g_index + 0];
	}
}

class RoboterPhase : public tw::TransWorhpProblem {
public:
	static std::string Text[];
	int mode;

	RoboterPhase(int dis, int mode) : TransWorhpProblem(tw::TWdimension(Text[mode].c_str(), dis, 10, 3, 1, 0, 1)), mode(mode) {
...
//		gr->ThreeD("3D");
	...
	void ode(double *dx, double t_, const double *x, const double *u, const double *p) override {
...
// Optional
	...
	*/
	...
	bool neben_diff(tw::DiffStructure &s, double t, const double *x, const double *u, const double *p) override {
	...
	bool neben_diff_p(tw::DiffStructure &s, double t, const double *x, const double *u, const double *p, int index) override {
...
std::string RoboterPhase::Text[] = {"Roboter Simulation",
							   "Roboter Momente",
