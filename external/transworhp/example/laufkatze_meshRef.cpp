...
public:
	...
	LaufkatzePhase(const tw::TWdimension &TWdata, XMLNode *xmlmain) : TransWorhpProblem(TWdata), grav(9.81) {
		freetime = true;
	...
	void selectWindows(tw::Viewer *viewer) override {
		...
		/*viewer->AddLambdaView(x_indexode(0),"x0");
		viewer->AddLambdaView(u_indexode(1),"u1");
	...
	void var_boundary(double *x_low, double *x_upp) override {
		...
		const double start[] = {0,0,0,0,0,5,0,0};
		const double ziel[] =  {8,0,0,0,0,4,0,0};
...
int main(int argv, char* argc[]) {
	...
	LaufkatzePhase ph(TWdim, xml_laufkatze.get());
	...
	for (int i = 0; i < twparameter.NDIS; i++) {
		const double z = i/(twparameter.NDIS-1.);
