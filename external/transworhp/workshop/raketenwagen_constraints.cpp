/*----------------------------------------------------------------
 *
 *  Workshop: Raketenwagen mit Nebenbedingung + Phasenplot
 *
 *----------------------------------------------------------------*/
...
double raketenplot (int &len, int &ndgl, int &nsteuer, double *t,double *x, double *u, int &i, int &index) {
	if (index == 0) {
		return x[i*(ndgl+nsteuer)+0];
	} else if (index == 1) {
		return x[i*(ndgl+nsteuer)+1];
	} else {
		return 0;
	}
}
...
public:
	...
	void OpenWindows(tw::Viewer *viewer) override {
		viewer->PhasePlot("Phasenplot", raketenplot, 0,1);
	}
	...
	bool neben_diff(tw::DiffStructure &s, double t, const double *x, const double *u, const double *p) override {
		s(0, x_indexode(0)) = 2*(x[0]-2);
		s(0, x_indexode(1)) = 2*(x[1]+2);
