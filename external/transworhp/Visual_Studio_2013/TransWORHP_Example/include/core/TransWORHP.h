...
public:
	...
	*/
	...
	void PrintOCPstates(std::ostream *os=nullptr);
	void PrintMultipliers(std::ostream *os=nullptr);
...
public:
	...
	virtual int DF_structure(WorhpMatrix *DF=nullptr, int offset=0);
	virtual void DF_calculate(WorhpMatrix &DF, double ScaleObj);

	virtual int DG_structure(const TWfolder *f, WorhpMatrix *DG=nullptr, int offset=0);
	virtual void DG_calculate(TWfolder *f, WorhpMatrix &DG);

	virtual int HM_structure(int hessianstructure, WorhpMatrix &DF, WorhpMatrix &DG, WorhpMatrix *HM=nullptr, int offset=0);
	virtual int HM_structure_ohne_Diag(int hessianstructure, WorhpMatrix &DF, WorhpMatrix &DG, WorhpMatrix *HM=nullptr, int offset=0);
...
public:
	...
	* @return Gitterpunkte als Vektor
	*/
	void newGrid(std::vector<double> const &grid);
...
class EmptyTransWorhp : public TransWorhp {
public:
	EmptyTransWorhp(const char *s, int dis, int ode, int ctrl, int param);
