...
public:
	...
	*/
	ExplTransWorhp(const char *s, int dis, const std::vector<int> &multinode, int ode, int ctrl, int param, int rand, int neben, const std::vector<int> &BOXneben=std::vector<int>(), int integral=0, int zen=0);
	...
	*/
	...
	void FromMATLAB(const std::string& filename) override;
