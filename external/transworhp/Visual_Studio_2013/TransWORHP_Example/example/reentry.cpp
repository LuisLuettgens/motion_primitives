...
public:
	...
	ReentryPhase(int dis) : TransWorhp("Reentry-Problem",dis,4,1,1,0,0) {
		freetime = 1;
...
class MyFolder : public TWfolder {

	public:

		MyFolder(TWparameter *twp, int a) : TWfolder(twp,a) {}
		bool step() override {

			for (int i=0;i<phases[0]->n_dis;i++) {
				while (phases[0]->u(i,0)>3*M_PI/2) phases[0]->X[phases[0]->u_index(i,0)] -= 2*M_PI;
				while (phases[0]->u(i,0)<-M_PI/2) phases[0]->X[phases[0]->u_index(i,0)] += 2*M_PI;
			}
			return true;
		}
};
