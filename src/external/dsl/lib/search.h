...
public:
    ...
    void SetDstarMin ( bool dstarMin ) {
        this->dstarMin = dstarMin;
    }

    void SetGoalBiad ( bool goalBias ) {
        this->goalBias = goalBias;
    }

    bool GetDstarMin() const {
        return dstarMin;
    }

    bool GetGoalBiad() const {
        return goalBias;
    }
...
void Search< Tv, Te >::ComputeShortestPath()
{
    ...
    if ( fibheap_empty ( openList ) ) {
        std::cerr << "[W] Search::ComputeShortestPath: openList is empty -- most "
                  ...
                  "goal!" << std::endl;
        std::cout << "[W] Search::ComputeShortestPath: openList is empty -- most "
                  "likely this means that a there is no path between start and "
                  "goal!" << std::endl;
        return;
    }
