...
Model1v1::Model1v1 ( string xmlparams ) :BaseModel ( xmlparams ) {
...
void Model1v1::ode ( double *dx, const double *x, const double *ctrl, const double *p, const double *q ) {
	x0        = x[0];
	y0        = x[1];
	psi       = x[2];
	u         = x[3];
...
void Model1v1::ode_structure ( tw::DiffStructure &s, int N ) {
	s ( N+0, N+2 );
	s ( N+0, N+3 );
	s ( N+0, N+4 );
	s ( N+1, N+2 );
...
void Model1v1::loadParams ( string xmlparams ) {
	XMLParser p;
...
void Model1v1::get_bounds ( double *low, double *upp ) {
	low[0]  =  nan ( "" );
	low[1]  =  nan ( "" );
	low[2]  =  nan ( "" );
	low[3]  =  nan ( "" );
...
void Model1v1::get_diff_bounds ( double *low, double *upp ) {
	low[0] = -3.*M_PI/180;
	low[1] = -3.*M_PI/180;
	low[2] = -0.01;
	low[3] = -0.01;
...
int Model1v1::get_nbrStates() {
//     return 8;
	return 10;
}

int Model1v1::get_nbrControls() {
	return 4;
}
