...
double Model1v5:: asign(double x) {
	return tanh(10000*x);
}

Model1v5::Model1v5 ( string xmlparams ) :BaseModel ( xmlparams ), xmlparams(xmlparams) {
...
void Model1v5::ode ( double *dx, const double *x, const double *ctrl, const double *p, const double *q ) {
...
// 	QE = c1*EOT[0]*EOT[0] + c2*n[0]*n[0] + c3*EOT[0]*n[0] + c4*EOT[0] + c5*n[0] + c6 + c7*EOT[0]*EOT[0]*EOT[0] + c8*n[0]*n[0]*n[0];
	...
	double Xtotal = X+XHR+XHL+2*XP1+0*XP2+2*XR+XA;//
	double Ytotal = Y+YHL+YHC+YP1+0*YP2+2*YR+YA;//
	double Ntotal = N+NHL+NHC+NP1+0*NP2+2*NR+NA;//
...
// 	cout << "x0: " << x[0] << "   y0: " << x[1] << "   psi: " << x[2] << "   u: " << x[3] << "   v: " << x[4] << "   r: " << x[5] << endl;
// 	cout << "XHR: " << XHR << "   XHL: " << XHL << "   XP1: " << XP1 << "   XP2: " << XP2 << "   XR: " << XR << "   XA: " << XA << endl;
// 	cout << "YHL: " << YHL << "   YHC: " << YHC << "   YP1: " << YP1 << "   YP2: " << YP2 << "   YR: " << XR << "   YA: " << YA << endl;
// 	cout << "NHL: " << NHL << "   NHC: " << NHC << "   NP1: " << NP1 << "   NP2: " << NP2 << "   NR: " << NR << "   NA: " << NA << endl;
// 	cout << "dx[3]: " << dx[3] << "   dx[4]: " << dx[4] << "   dx[5]: " << dx[5] << endl;
...
void Model1v5::get_paramAddr( vector<double*> &paramAddr ) {
...
void Model1v5::ode_structure ( tw::DiffStructure &s, int N ) {
	s ( N+0, N+2 );
	s ( N+0, N+3 );
	s ( N+0, N+4 );
	s ( N+1, N+2 );
...
void Model1v5::updateParams ( const double *p ) {
	int j=0;
...
void Model1v5::get_bounds ( double *low, double *upp ) {
	XMLParser p;
...
void Model1v5::get_param_bounds ( double *low, double *upp ) {
	XMLParser p;
	XMLNode *n = p.Parse ( xmlparams )->GetFirstChild ( "PARAMS" );
	XMLNode *xml = n->GetFirstChild ( "DOUBLE" );
...
void Model1v5::get_diff_bounds ( double *low, double *upp ) {
}


int Model1v5::get_nbrIdParams(  ) {
	int nbrParams = 0;
...
void Model1v5::get_paramIdfy   ( vector<bool> &paramIdfy  ) {
	XMLParser p;
	XMLNode *n = p.Parse ( xmlparams )->GetFirstChild ( "PARAMS" );
	XMLNode *xml = n->GetFirstChild ( "DOUBLE" );
...
void Model1v5::get_paramScales ( vector<double> &paramScales ) {
	XMLParser p;
	XMLNode *n = p.Parse ( xmlparams )->GetFirstChild ( "PARAMS" );
	XMLNode *xml = n->GetFirstChild ( "DOUBLE" );
...
void Model1v5::get_initialS ( vector<double> &initialS ) {
	XMLParser p;
